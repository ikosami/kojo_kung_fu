...
public class Stage3Event : MonoBehaviour
{
    ...
    [SerializeField] Shisico2 shisico;

    // カメラ移動シーケンスで使用する変数を保存
    ...
    void Start()
    {
        shisico.gameObject.SetActive(false);
    }

    public void OnStageEvent()
    {
        shisico.gameObject.SetActive(true);
    ...
    private IEnumerator MoveCameraSequence()
    {
        ...
        var pos = shisico.transform.position;
        pos.x = Reference.Instance.player.transform.position.x + 530;
        shisico.transform.position = pos;

        yield return new WaitForSeconds(1);

        stageRect = Reference.Instance.stageRect;
        playerRect = Reference.Instance.player.Rect;

        // 開始位置と目標位置を保存
        startPos = stageRect.anchoredPosition;
    ...
    private IEnumerator MoveCameraSequence2()
    {
        // フェーズ3: 元の位置に戻る
